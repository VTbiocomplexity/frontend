...
class AppStateStub {
  ...
  getUser(uid) {
    if (uid === '1') {
      this.user = {
        _id: '1', userType: 'Developer', name: 'Adrean Elba', email: 'one@one.com', rafterApps: [{ r_app_name: 'yo' }, { r_app_name: 'slow' }]
      };
    } else if (uid === '2') {
      this.user = {
        _id: '2', userType: 'Developer', name: 'Bogus Elba', email: 'two@two.com', rafterApps: [{ r_app_name: 'yo' }, { r_app_name: 'slow' }]
      };
    } else {
      this.user = {
        name: 'Cathy Elba', userType: 'Developer', email: 'three@three.com', rafterApps: [{ r_app_name: 'yo' }, { r_app_name: 'slow' }]
      };
    }
...
class HttpMock {
  ...
  fetch(url, obj) {
    ...
    if (url === '/rafter/rlogin') {
      ...
      if (!this.error) {
        const data = '{authorization_token: 123}, {&#34;user&#34;: {&#34;name&#34;: &#34;tester&#34;}';
        return Promise.resolve({
          Headers: this.headers,
    ...
    if (url === '/rafter/vs') {
      ...
      if (!this.error && !this.message) {
        ...
        const em = JSON.stringify({ error: 'you did something wrong' });
        const data = { message: em };
        return Promise.resolve({
          Headers: this.headers,
          json: () => Promise.resolve(data)
        });
